...
. $DIR/share.sh batch_delete_file $@
...
pdm template delete_file --sessions $total --bursts 1 < $factors > $pressure
...
echo Analysis logs
pdm analysis delete_file $working/*.log > $result -t $now $db_opts
