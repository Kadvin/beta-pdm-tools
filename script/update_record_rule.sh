...
. $DIR/share.sh update_record_rule $@

echo Retrieve rule keys...
pdm userpane list record_rules -f key > $factors -s $server

echo Generate templates
pdm template update_record_rule --sessions $total --bursts 1 < $factors > $pressure
...
echo Analysis logs
pdm analysis update_record_rule $working/*.log > $result -t $now $db_opts
